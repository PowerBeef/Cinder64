...
async fn main() -> std::io::Result<()> {
    ...
    if let Some(game) = args.game {
        ...
        let recent_rom = ui::config::RecentRom {
            path: game.clone(),
            title: file_path
                .file_name()
                .and_then(|name| name.to_str())
                .map_or_else(|| game.clone(), ToString::to_string),
            last_launched_unix: std::time::SystemTime::now()
        ...
        device
            ...
            .config
            .recent_roms
            .retain(|entry| entry.path != recent_rom.path);
        device.ui.config.recent_roms.insert(0, recent_rom);
        device.ui.config.recent_roms.truncate(10);
