...
static bool initial_paused;
static std::atomic_bool callback_emu_running = false;
static std::atomic_bool callback_save_state = false;
static std::atomic_bool callback_load_state = false;
static std::atomic_bool callback_reset_game = false;
static std::atomic_bool callback_enable_speedlimiter = false;
static std::atomic_bool callback_lower_volume = false;
static std::atomic_bool callback_raise_volume = false;
static std::atomic_bool callback_paused = false;
static std::atomic_bool callback_frame_advance = false;
static std::atomic_uint32_t callback_save_state_slot = 0;
static std::atomic_uint32_t host_surface_width = 640;
...
static CALL_BACK snapshot_callback() {
  CALL_BACK result = {};
  result.emu_running = callback_emu_running.load();
  result.save_state = callback_save_state.exchange(false);
  result.load_state = callback_load_state.exchange(false);
  result.reset_game = callback_reset_game.exchange(false);
  result.enable_speedlimiter = callback_enable_speedlimiter.load();
  result.lower_volume = callback_lower_volume.exchange(false);
  result.raise_volume = callback_raise_volume.exchange(false);
  result.paused = callback_paused.load();
  result.frame_advance = callback_frame_advance.exchange(false);
  result.save_state_slot = callback_save_state_slot.load();
  return result;
}
...
bool sdl_event_filter(void *userdata, SDL_Event *event) {
  if (event->type == SDL_EVENT_WINDOW_CLOSE_REQUESTED) {
    callback_paused.store(false);
    callback_emu_running.store(false);
  } else if (event->type == SDL_EVENT_WINDOW_RESIZED && callback_emu_running.load()) {
    ...
    case SDL_SCANCODE_F:
      if (event->key.mod & SDL_KMOD_ALT) {
        callback_enable_speedlimiter.store(!callback_enable_speedlimiter.load());
      }
      break;
    case SDL_SCANCODE_P:
      if (event->key.mod & SDL_KMOD_ALT) {
        callback_paused.store(!callback_paused.load());
      }
      break;
    ...
    case SDL_SCANCODE_F5:
      callback_save_state.store(true);
      break;
    case SDL_SCANCODE_F7:
      callback_load_state.store(true);
      break;
    ...
    case SDL_SCANCODE_F12:
      callback_reset_game.store(true);
      break;
    case SDL_SCANCODE_LEFTBRACKET:
      callback_lower_volume.store(true);
      break;
    case SDL_SCANCODE_RIGHTBRACKET:
      callback_raise_volume.store(true);
      break;
    case SDL_SCANCODE_SLASH:
      callback_frame_advance.store(true);
      break;
    case SDL_SCANCODE_0:
    case SDL_SCANCODE_1:
    case SDL_SCANCODE_2:
    ...
    case SDL_SCANCODE_9:
      if (event->key.mod & SDL_KMOD_ALT) {
        if (event->key.scancode == SDL_SCANCODE_0)
          callback_save_state_slot.store(0);
        else
          callback_save_state_slot.store(event->key.scancode - SDL_SCANCODE_1 + 1);
      }
      break;
    default:
      break;
...
void rdp_init(void *_window, GFX_INFO _gfx_info, const void *font,
  ...
  callback_emu_running.store(true);
  callback_enable_speedlimiter.store(true);
  callback_paused.store(initial_paused);
  callback_save_state_slot.store(save_state_slot);
  callback_save_state.store(false);
  callback_load_state.store(false);
  callback_reset_game.store(false);
  callback_lower_volume.store(false);
  callback_raise_volume.store(false);
  callback_frame_advance.store(false);
...
void rdp_close() {
  ...
  run_close_step("end_frame", [&] {
    if (wsi)
      wsi->end_frame();
  });

  run_close_step("message_font", [&] {
    if (message_font) {
      TTF_CloseFont(message_font);
      message_font = nullptr;
    }
  });
  run_close_step("achievement_font", [&] {
...
const char *rdp_last_close_error() {
  return last_close_error.empty() ? nullptr : last_close_error.c_str();
}

void rdp_set_initial_paused(bool paused) { initial_paused = paused; }

void rdp_set_paused(bool paused) { callback_paused.store(paused); }

void rdp_request_save_state(uint32_t slot) {
  callback_save_state_slot.store(slot);
  callback_save_state.store(true);
}

void rdp_request_load_state(uint32_t slot) {
  callback_save_state_slot.store(slot);
  callback_load_state.store(true);
}

void rdp_request_reset() { callback_reset_game.store(true); }
...
static void calculate_viewport(float *x, float *y, float *width, float *height,
  ...
  float w = static_cast<float>(host_surface_width.load());
  float h = static_cast<float>(host_surface_height.load());

  if (gfx_info.integer_scaling) {
    // Integer scaling path
    int scale_x = static_cast<int>(w) / static_cast<int>(display_width);
    int scale_y = static_cast<int>(h) / static_cast<int>(display_height);
    ...
    *height = scaled_height;
    ...
    // Center the viewport
    int integer_x = static_cast<int>((w - *width) / 2.0f);
    int integer_y = static_cast<int>((h - *height) / 2.0f);
