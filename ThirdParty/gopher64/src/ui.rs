...
static HOST_DIR_OVERRIDE: OnceLock<Mutex<Option<Dirs>>> = OnceLock::new();
...
pub fn set_dirs_override(dirs: Option<Dirs>) {
    let mut override_dirs = HOST_DIR_OVERRIDE
...
pub fn get_dirs() -> Dirs {
    if let Some(dirs) = HOST_DIR_OVERRIDE
        .get_or_init(|| Mutex::new(None))
        .lock()
        .unwrap()
        .clone()
