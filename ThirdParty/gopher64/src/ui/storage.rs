...
pub fn persist_data(path: std::path::PathBuf, data: Vec<u8>) {
...
fn write_save(ui: &mut ui::Ui, save_type: SaveTypes) {
    ...
    persist_data(save_path, save_data);
}
...
mod tests {
    ...
    fn unique_path(label: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("cinder64-{label}-{unique}.bin"))
    }

    #[test]
    fn write_save_without_tokio_runtime_persists_to_disk() {
        let mut ui = ui::Ui::default();
        let save_path = unique_path("save");
        ui.storage.paths.eep_file_path = save_path.clone();
        ui.storage.saves.eeprom.data = vec![1, 2, 3, 4];
        ui.storage.saves.eeprom.write_pending = true;

        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            write_save(&mut ui, SaveTypes::Eeprom4k);
        }));

        assert!(
            result.is_ok(),
            "write_save should not require a Tokio runtime"
        );
        assert_eq!(std::fs::read(&save_path).unwrap(), vec![1, 2, 3, 4]);
