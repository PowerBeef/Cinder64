...
const HOST_SCANCODE_COUNT: usize = 512;

static HOST_KEYBOARD_STATE: OnceLock<Mutex<[bool; HOST_SCANCODE_COUNT]>> =
    OnceLock::new();
...
fn host_keyboard_state() -> &'static Mutex<[bool; HOST_SCANCODE_COUNT]> {
    HOST_KEYBOARD_STATE.get_or_init(|| Mutex::new([false; HOST_SCANCODE_COUNT]))
}

pub fn set_host_key_state(scancode: i32, pressed: bool) {
    if scancode < 0 || scancode >= HOST_SCANCODE_COUNT as i32 {
        return;
    }

    if let Ok(mut state) = host_keyboard_state().lock() {
        state[scancode as usize] = pressed;
    }
}

pub fn reset_host_key_state() {
    if let Ok(mut state) = host_keyboard_state().lock() {
        state.fill(false);
    }
}

fn is_key_pressed(keyboard_state: *const bool, scancode: i32) -> bool {
    let host_pressed = host_keyboard_state()
        .lock()
        .map(|state| {
            if scancode < 0 || scancode >= HOST_SCANCODE_COUNT as i32 {
                false
            } else {
                state[scancode as usize]
            }
        })
        .unwrap_or(false);

    let sdl_pressed = if keyboard_state.is_null()
        || scancode < 0
        || scancode >= HOST_SCANCODE_COUNT as i32
    {
        false
    } else {
        unsafe { *keyboard_state.offset(scancode as isize) }
    };

    host_pressed || sdl_pressed
}
...
fn set_axis_from_keys(
    ...
    let mut x = 0.0;
    let mut y = 0.0;
    if profile.keys[AXIS_LEFT].enabled && is_key_pressed(keyboard_state, profile.keys[AXIS_LEFT].id)
    {
        x = -MAX_AXIS_VALUE
    }
    if profile.keys[AXIS_RIGHT].enabled
        && is_key_pressed(keyboard_state, profile.keys[AXIS_RIGHT].id)
    {
        x = MAX_AXIS_VALUE
    }
    if profile.keys[AXIS_DOWN].enabled && is_key_pressed(keyboard_state, profile.keys[AXIS_DOWN].id)
    {
        y = -MAX_AXIS_VALUE
    }
    if profile.keys[AXIS_UP].enabled && is_key_pressed(keyboard_state, profile.keys[AXIS_UP].id) {
        y = MAX_AXIS_VALUE
    }
    (x, y)
}
...
fn change_paks(
    ...
    if controller_button.enabled && !controller.is_null() {
        ...
        pressed = is_key_pressed(keyboard_state, key.id);
    }
    pressed
}
...
pub fn get(ui: &ui::Ui, channel: usize) -> InputData {
    ...
    let alt_pressed =
        ...
        is_key_pressed(
            ...
            i32::from(sdl3_sys::scancode::SDL_SCANCODE_LALT),
        ) || is_key_pressed(
            ui.input.keyboard_state,
            i32::from(sdl3_sys::scancode::SDL_SCANCODE_RALT),
        );
    ...
    for i in 0..14 {
        if profile_name != "default" || channel == 0 {
            ...
            if profile_key.enabled && !alt_pressed {
                keys |= (is_key_pressed(ui.input.keyboard_state, profile_key.id) as u32) << i;
            }
        }
