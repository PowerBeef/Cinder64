...
fn write_shutdown_phase(ui: &ui::Ui, phase: &str) {
    let _ = fs::write(ui.dirs.cache_dir.join("shutdown-phase.txt"), phase);
}
...
pub fn run_game(device: &mut Device, rom_contents: &[u8], game_settings: ui::gui::GameSettings) {
    run_game_with_ready_hook(device, rom_contents, game_settings, |_| {})
}

pub fn run_game_with_ready_hook<F: FnOnce(&Device)>(
    device: &mut Device,
    rom_contents: &[u8],
    ...
    write_shutdown_phase(&device.ui, "before write_saves");
    ui::storage::write_saves(device);
    write_shutdown_phase(&device.ui, "before retroachievements shutdown");
    retroachievements::shutdown_client();
    write_shutdown_phase(&device.ui, "before input close");
    ui::input::close(&mut device.ui);
    write_shutdown_phase(&device.ui, "before audio close");
    ui::audio::close(&mut device.ui);
    write_shutdown_phase(&device.ui, "before video close");
    ui::video::close(&device.ui);
    write_shutdown_phase(&device.ui, "after video close");
}
